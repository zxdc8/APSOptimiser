...
objFun=@(x)objectivepso2(x,iter);
constFun = @(x)constraints(x,iter);


%Interior Point Algorithm
options = optimoptions('fmincon','Algorithm','interior-point','Display','iter-detailed','FiniteDifferenceStepSize',1,'FunctionTolerance',1e-3,'StepTolerance',1e-9,'PlotFcns',@optimplotfval,'MaxIterations',40,'UseParallel',true);


%Run Optimisation
[X,J,EXITFLAG,OUTPUT]=fmincon(objFun,x0,[],[],[],[],LB,UB,constFun,options);   
